...
if($condicion=='table1'){
	...
	if($filtrado!=''){
		$filtrado = ' and (preo.texto LIKE "%'.$filtrado.'%")';
	}

	if($sede!=''){
		$sede = ' and preo.preguntas = '.$sede;
	}

	$limit = $consultasporpagina;
	$offset = ($pagina - 1) * $consultasporpagina;

	$sql1 = "SELECT preo.id as preo_id, preo.texto as preo_texto, preo.preguntas as preo_preguntas, preo.orden as preo_orden, preo.fecha_creacion as preo_fecha_creacion, pre.id as pre_id, pre.texto as pre_texto FROM preguntas_opciones preo 
		INNER JOIN preguntas pre 
		ON preo.preguntas = pre.id 
		WHERE preo.id != 0 
		".$filtrado." 
		".$sede."
	";
	
	$sql2 = "SELECT preo.id as preo_id, preo.texto as preo_texto, preo.preguntas as preo_preguntas, preo.orden as preo_orden, preo.fecha_creacion as preo_fecha_creacion, pre.id as pre_id, pre.texto as pre_texto FROM preguntas_opciones preo 
		INNER JOIN preguntas pre 
		ON preo.preguntas = pre.id 
		WHERE preo.id != 0 
		".$filtrado." 
	...
	if($conteo1>=1){
		while($row2 = mysqli_fetch_array($proceso2)) {
			...
			$html .= '
		                <tr id="tr_'.$row2["preo_id"].'">
		                    <td style="text-align:center;">'.$row2["preo_id"].'</td>
		                    <td style="text-align:center;">'.$row2["preo_texto"].'</td>
		                    <td style="text-align:center;">'.$row2["pre_texto"].' -> ['.$row2["preo_id"].']</td>
		                    <td style="text-align:center;">'.$row2["preo_orden"].'</td>
		                    <td style="text-align:center;">'.$row2["preo_fecha_creacion"].'</td>
		                    <td class="text-center" nowrap="nowrap">
		                    	<button class="btn btn-danger" onclick="eliminar1('.$row2["preo_id"].');">Eliminar</button>
		                    </td>
		                </tr>
			';
		}
